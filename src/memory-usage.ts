...
export function processMemoryUsage(
  ...
  const rss = telemetry.createUpDownCounter({
    name: namePrefix + 'memory_rss_bytes',
    description:
  ...
  const arrayBuffers = telemetry.createUpDownCounter({
    name: namePrefix + 'array_buffers_bytes',
    description:
  ...
  telemetry.createBatchObserver({ name: namePrefix + 'memory_usage', description, labels }, () => {
